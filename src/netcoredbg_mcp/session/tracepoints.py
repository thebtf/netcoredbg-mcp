...
class TracepointManager:
    ...
    def find_tracepoint_for_location(self, file: str, line: int) -> Tracepoint | None:
        ...
        for tp in self._tracepoints.values():
            if tp.active and tp.file.replace("\\", "/").lower() == normalized and tp.line == line:
                return tp
        return None
    ...
    async def on_tracepoint_hit(
        ...
        """Handle a tracepoint hit: evaluate expression, resume, log result.

        Called from SessionManager._on_stopped when a tracepoint is detected.
        ...
        # Resume execution (always — never leave paused due to tracepoint)
        try:
            session.prepare_for_execution()
            await session._client.continue_execution(thread_id)
        except Exception as e:
