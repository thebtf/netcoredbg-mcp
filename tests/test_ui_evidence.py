...
@pytest.mark.asyncio
async def test_ui_grid_double_click_row_uses_backend_row_click_after_identity_resolution(
    capturing_mcp,
    monkeypatch,
) -> None:
    class KeyedGridBackend(FakeEvidenceBackend):
    ...
    assert response["data"]["status"] == "PASS"
    assert response["data"]["canonical_action"] == "double_click_row"
    assert response["data"]["clicked"] is True
...
@pytest.mark.asyncio
async def test_ui_grid_click_row_can_opt_in_to_ensure_visible(
    capturing_mcp,
    monkeypatch,
) -> None:
    class EnsureThenClickBackend(FakeEvidenceBackend):
        def __init__(self) -> None:
            super().__init__()
            self.process_id = 42
            self.snapshot_calls: list[dict[str, Any]] = []
    ...
    session.state.state = DebugState.RUNNING
    session.state.process_id = 42
    backend = EnsureThenClickBackend()
    ...
    assert response["data"]["status"] == "PASS"
    assert response["data"]["canonical_action"] == "click_row"
    assert response["data"]["ensure_visible_result"] == {
        "status": "PASS",
        "realized": True,
    ...
    assert len(backend.snapshot_calls) == 3
...
def test_manual_smoke_list_includes_focused_ui_evidence_scenario() -> None:
    ...
    extended_result = subprocess.run(
        [sys.executable, "tests/smoke_test_manual.py", "--list", "--extended-gui"],
        check=False,
        capture_output=True,
        text=True,
    )
