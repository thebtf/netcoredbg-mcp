...
@pytest.mark.asyncio
async def test_ui_operation_adapters_drag_blocks_when_target_ensure_visible_hides_source() -> None:
    class FakeBackend:
        def __init__(self) -> None:
            self.drag_calls: list[tuple[int, int, int, int, int, list[str]]] = []
            self.grid_ensure_visible_calls: list[dict[str, Any]] = []
                ...
                "visible_rows": [
                    {
                        "index": 18,
                        "bounds": {"x": 10, "y": 340, "width": 120, "height": 30},
                        "cells": {"Phrase": "Cue target eighteen"},
                    }
                ],
            }
    ...
    selector = {"automation_id": "CueDataGrid"}
    result = await ui_operation_adapters(backend_provider)["ui.drag"](
        source={"kind": "row_identity", "selector": selector, "row_identity": "Cue source"},
        path=[
            {"relative_to": "source", "x": 0.5, "y": 0.5},
            {"relative_to": "drop", "x": 0.5, "y": 0.5},
        ],
    ...
    assert result["status"] == "BLOCKED"
    assert result["reason"] == "drag source row no longer visible after target ensure-visible"
    assert backend.grid_ensure_visible_calls == [
        {
            "selector": selector,
            "row_key": "Cue target eighteen",
            "row_index": None,
            "identity": {"column": "Phrase"},
            "rows": {"visible_only": True},
            "columns": ["Phrase"],
...
@pytest.mark.asyncio
async def test_ui_drag_blocks_when_target_ensure_visible_hides_shorthand_source() -> None:
    class FakeBackend:
        def __init__(self) -> None:
            self.drag_calls: list[tuple[int, int, int, int, int, list[str]]] = []
            self.grid_ensure_visible_calls: list[dict[str, Any]] = []
...
async def test_ui_operation_adapters_grid_double_click_row_requires_backend_click_primitive(
    ...
    result = await ui_operation_adapters(backend_provider)["ui.grid.double_click_row"](
        selector={"automation_id": "dataGrid"},
        row={"identity": "Cue 019"},
        identity={"column": "PhraseId"},
        column="PhraseId",
    ...
    assert result["status"] == "BLOCKED"
    assert result["reason"] == "grid row double click unavailable"
    assert result["requested"] == {"adapter": "ui.grid.double_click_row"}
