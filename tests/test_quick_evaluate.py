...
class TestQuickEvaluate:
    ...
    def manager(self):
        with patch("netcoredbg_mcp.session.manager.DAPClient"):
            ...
            m._client.continue_execution = AsyncMock(
                return_value=DAPResponse(seq=3, request_seq=3, success=True, command="continue")
            )
            m.get_stack_trace = AsyncMock(
                return_value=[StackFrame(id=1, name="Main", source="test.cs", line=10, column=1)]
            )
    ...
    async def test_quick_evaluate_eval_error(self, manager):
        ...
        manager._client.evaluate = AsyncMock(
            return_value=DAPResponse(
                seq=2,
                request_seq=2,
                success=False,
