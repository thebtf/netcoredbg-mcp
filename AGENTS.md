...
| **Exact-head SonarQube release gate** | Before merge or tag creation, scan the final release-candidate `HEAD_SHA` using `SonarQube.Analysis.xml`; retain the scanner task, `SUCCESS` Compute Engine, analysis-bound `OK` quality gate, and per-finding disposition receipt. Every finding must be fixed on that same head; `NOSONAR`, suppression, `WONTFIX`, `FALSE-POSITIVE`, accepted risk, or a stale/latest-project result never bypasses release. |
| **Sonar readiness is fail-closed** | `SONAR_HOST_URL`, `SONAR_TOKEN`, and a usable configured scanner are mandatory. Missing, blank, inaccessible, rejected, or unsafe-to-expose credentials are `SONAR_CREDENTIALS_UNAVAILABLE` blocker evidence (record input names only, never values); do not infer a pass, merge, tag, or publish. |
...
7. After the final review correction, read the release candidate `HEAD_SHA`, run the exact-head SonarQube gate from `docs/RELEASE-PROTOCOL.md`, and record its scanner/task/analysis/quality-gate/finding receipt. Fix every SonarQube finding in code and rescan the new exact head until every finding is closed as `FIXED_IN_CURRENT_HEAD`; every such source correction also requires the affected review and required checks to be rerun before the final scan. A missing credential, scanner/task/analysis mismatch, failed or nonterminal Compute Engine task, non-`OK` quality gate, or any unresolved finding blocks release.
8. Merge the PR automatically only when the primary UXDD consumer-mode release gate, review, SonarQube exact-head receipt, and required checks are clean unless a high-risk approval trigger above applies.
9. Update local `main`, verify the merged release commit, create and push the annotated PATCH/MINOR tag automatically, then run post-publication verification per `docs/RELEASE-PROTOCOL.md`. Do not wait for a separate user command.
