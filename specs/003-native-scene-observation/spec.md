...
At commit, the server computes the complete artifact SHA-256 recorded in the manifest and a server-internal SHA-256 table over fixed 65,536-byte storage chunks. Before every authorized read, including the first, it verifies immutable file identity and length and re-hashes every storage chunk touched by the requested byte range; an unaligned range crossing a chunk boundary verifies both chunks before releasing bytes. Any full-hash, touched-chunk, identity, or length mismatch returns `ARTIFACT_INTEGRITY_FAILED` and emits no bytes. A valid debug session that requests an unknown, foreign-session, foreign-capture, expired, deleted, or otherwise unavailable `artifactId` gets the same `ARTIFACT_NOT_FOUND` result; that result discloses no existence, ownership, retention, size, hash, provenance, or path. Invalid syntax/bounds are `INVALID_TOOL_ARGUMENTS` before lookup.
...
- **FR-012 — Artifact authorization and integrity:** Foreign, expired, deleted, and unknown artifact IDs return indistinguishable `ARTIFACT_NOT_FOUND` to an otherwise valid session. Commit computes the manifest SHA-256 and an internal fixed-65,536-byte chunk-hash table. Every authorized read verifies immutable identity/length and all touched chunks before release, including both chunks for an unaligned crossing. Any mismatch returns `ARTIFACT_INTEGRITY_FAILED` without bytes.
...
| Add bounded observer/probe lifecycle, immutable artifact storage, and artifact reader | Code | Native front door | Focused tests prove correlation, one in-flight operation, cancellation cleanup, atomic writes, opaque IDs, negotiated chunk limit, base64 offset reads, commit-time full/chunk hashes, every-read identity/length plus touched-chunk verification, unaligned two-chunk coverage, and typed write/not-found/integrity failures with zero bytes on mismatch. |
...
- **AC-005:** M0 `read_capture_artifact` returns only bounded standard-base64 chunks by zero-based offset and negotiated `maxBytes`, never full capture artifacts or a root/path. Commit computes full and fixed-65,536-byte chunk hashes; every read verifies identity/length and every touched chunk before releasing bytes, including unaligned two-chunk ranges.
- **AC-006:** M0 proves that a valid session receives indistinguishable `ARTIFACT_NOT_FOUND` with no metadata for foreign, expired, and nonexistent artifact capabilities; it proves `ARTIFACT_INTEGRITY_FAILED` and zero emitted bytes for full-hash, touched-chunk, identity, or length mismatch, including same-identity/same-length in-place tampering after a prior successful read.
