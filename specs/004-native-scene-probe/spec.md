...
**Independent Test**: Capture visual evidence from an authorized session, retrieve it in several bounded portions, reassemble it, and verify that it matches the capture manifest. After one successful read, tamper an authorized artifact in place without changing its identity or length, then request an unaligned two-chunk range and verify containment.
...
4. **Given** an authorized artifact that was successfully read and whose committed bytes are then changed in place without changing file identity or length, **When** a requested range touches one or more 65,536-byte storage chunks, **Then** identity, length, and every touched chunk hash are verified before release; a mismatch returns an integrity failure and no bytes.
...
- An authorized artifact was successfully read, then is modified in place without changing file identity or length: every later authorized read rechecks identity, length, and each touched fixed 65,536-byte storage chunk before it releases any bytes; a mismatch returns no bytes and a distinct integrity failure.
...
- **FR-002**: The approved contract MUST define the complete evidence vocabulary, required and optional information, versions, limits, success outcomes, qualified outcomes, tool-specific permitted error outcomes, and compatibility behavior for every M0 and M1 user outcome. Each declared primitive name MUST be structurally bound to its fixed M0 or M1 milestone and cannot pair with the other milestone.
...
- **FR-005**: Before a consumer relies on observation behavior, the feature MUST declare supported contract versions, available observation capabilities, supported context conditions, limits, and candidate identity. Supported protocol/schema version arrays MUST be non-empty, unique, and structurally contain `native-scene-probe/1` and `native-scene-probe.schema/1`, respectively.
- **FR-006**: M0 visual capture MUST be read-only, bounded, and attributable. It MUST produce lossless visual evidence plus a compact manifest rather than embedding the lossless evidence in the capture response. Its manifest MUST include the non-null immutable `evidenceScope` supplied by the request, equal at runtime to that request scope; element and native-scene manifests MUST use `evidenceScope: null`. A `COMPLETE` visual-capture result MUST include at least one descriptor with `mediaType` `image/png` and `evidenceGrade` `lossless_visual`; `PARTIAL` and `UNOBSERVABLE` may contain zero artifacts only when evidence could not be committed.
- **FR-007**: Each visual or scene artifact MUST be staged and committed immutably under server ownership, described by an opaque unguessable capability bound to its session and capture, and accompanied by media type, length, full SHA-256 integrity value, contract version, capture identity, and retention information. At commit the server MUST also compute an internal SHA-256 table for fixed 65,536-byte storage chunks; the table is not public manifest data or caller authority.
- **FR-008**: Artifact retrieval MUST accept only the owning session, an artifact capability, and a bounded range; return no more raw content than the agreed bound; preserve exact byte order; and disclose neither a storage location nor a storage-root identifier. Returned `captureArtifactChunk.byteLength` MUST be at most 16,777,216 for `application/vnd.netcoredbg.native-scene+json` and at most 67,108,864 for raster media types.
- **FR-009**: Every authorized artifact retrieval MUST verify immutable file identity and length, then re-hash every fixed 65,536-byte storage chunk touched by the requested range before releasing data, including an unaligned range spanning two chunks. The full public manifest SHA-256 and internal committed chunk table are both immutable commit facts. A failed authorized integrity check MUST return no bytes and a distinct integrity-failure outcome.
...
- **SC-003**: In 100% of accepted authorization trials, foreign, expired, deleted, and nonexistent artifact capabilities produce the same unavailable-artifact result with zero disclosed artifact metadata; in 100% of accepted tamper trials—including in-place same-identity/same-length tampering after a prior successful read and an unaligned two-chunk request—integrity failure returns zero artifact bytes.
