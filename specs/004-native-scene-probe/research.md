...
This research resolves the implementation inputs for the six additive M0/M1 primitives. It authorizes **planning only**. T001 may authorize only M0-G0 contract/parity work T002–T007; M0 implementation remains blocked until GREEN T007. GREEN T007 is limited to exact-byte schema loading, contract/runtime-validator parity on concrete fixtures, corpus syntax/reference integrity, classification vocabulary, and negative structural/version cases; it is not observer, artifact, stability, or atomicity behavior evidence. The checked-in schemas remain design candidates until the operator accepts the M0-G0 gate; this document does not authorize product code or package edits.
...
| What freezes the wire? | Spec 003 and ADR-003 call the examples semantic sketches. T001 operator approval authorizes only M0-G0 T002–T007; before any M0 implementation T008+, T007 must accept exact Draft-7 bytes, request/result contract-validator parity on concrete fixtures, valid corpus syntax/internal references, declared classification vocabulary, and negative structural/version cases. | **ADOPT:** Draft 7/common keywords only; closed primitive roots and `sceneRequest`; each C001–C024 corpus entry declares `contractGateExpectation` and `runtimeBehaviorRequired`. **GATE:** operator approval after this planning packet, then GREEN T007 before M0 implementation. T007 does not execute observer/artifact/stability/atomicity behavior or require full C001–C024 behavioral GREEN; T032/T034 do. |
...
| How are artifacts stored and verified? | ADR-003 requires per-session server-owned roots, staged then atomic writes, immutable committed artifacts, opaque session/capture-bound IDs, a public full SHA-256, and an internal SHA-256 table for fixed 65,536-byte storage chunks. Every authorized read verifies identity/length and re-hashes every touched chunk before release. BCL `FileStream`, `SHA256`, and `RandomNumberGenerator` cover storage, integrity, and opaque IDs. | **ADOPT:** BCL file I/O, commit-time full/chunk hashes, CSPRNG capability IDs, bounded reads with touched-chunk verification. **REJECT:** caller-selected destinations, absolute-root disclosure, path reads, mutable artifact replacement, and exposing the chunk table. |
...
| Integrity and IDs | [`SHA256`](https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.sha256) supports hashing streams; [`RandomNumberGenerator`](https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.randomnumbergenerator) provides cryptographically strong random values. | At commit hash the full artifact for the public manifest and hash every fixed 65,536-byte storage chunk in a server-internal table. Before every authorized release, verify file identity/length and re-hash every touched chunk; identity, length, or chunk mismatch returns `ARTIFACT_INTEGRITY_FAILED` and no bytes. |
...
- protocol `native-scene-probe/1`, explicit schema version, closed request roots and `sceneRequest`, plus an immutable manifest `evidenceScope`: visual is non-null and runtime-equal to the request scope; element/native-scene are `null`;
...
- schema limits: artifact read bytes `1..65536`; lossless artifact and raster chunk metadata `<=64 MiB`; scene artifact and native-scene chunk metadata `<=16 MiB`; manifest/structured response `<=256 KiB`; `selectedState` and `currentState` each `<=262,144` serialized UTF-8 bytes before DTO materialization; graph `<=4096` nodes; issues `<=256`; artifact references `<=4`; settle timeout `<=30s`; retention `4h` or session stop; generally strings `1..256`, prose/error text `<=1024`; byte offsets bounded by `9007199254740991`;
- six primitives only: M0 `get_ui_probe_capabilities`, `capture_visual_evidence`, `read_capture_artifact`; M1 `wait_for_ui_stable`, `capture_element_snapshot`, `capture_native_scene`; the declaration's array has `minItems: 6`, `maxItems: 6`, and an `allOf`/`contains` constraint for each name. Each `primitiveCapability` is structurally constrained by `oneOf` fixed `{name, milestone}` pairs: the three M0 names map only to M0 and the three M1 names map only to M1; `supportedProtocolVersions` and `supportedSchemaVersions` are non-empty, unique, and contain their active `/1` value; runtime asserts the exact paired six-entry set with no duplicate or omission; no `check_element_tokens`;
- each declared settle condition—`dispatcherIdle`, `stableLayout`, `animationState`, `windowGeometry`, `contextMaterialization`, and `asyncLoadSettled`—has an explicit capability state; negotiated `settleSampleCountMin` is structurally `const: 2`, `settleSampleCountMax` is structurally `const: 16`, and request `sampleCount` remains 2–16;
- every element snapshot and native-scene evidence-returning or evidence-committing branch has capture-time `revalidatedByCapture: true`, including `PARTIAL` and `UNOBSERVABLE`; only a standalone wait receipt has `false`. Persisted `native_scene` `PARTIAL` artifacts require in-process or UIA-guarded atomicity; the UIA branch retains unchanged guards and its atomicity issue, while element artifacts retain `not_applicable`;
- `toolError` is six tool-specific one-of branches derived from the existing outcome table; only each primitive's mapped codes are accepted and cross-pairs are rejected. Unknown, foreign, expired, deleted, and unavailable artifact IDs instead use exactly `{kind: "tool_error", tool: "read_capture_artifact", code: "ARTIFACT_NOT_FOUND", message: "Artifact is not available."}` with no additional member, artifact metadata, or free-text variation;
- schemas are checked-in **design candidates** until T001 approval. M0-G0 T002–T007 validates only exact-byte loading, contract/runtime-validator parity, corpus syntax/reference integrity, expected classification vocabulary, scope/version/error/chunk structural negatives, and negative structural/version cases. Full behavior, including C005–C016/C020 runtime behavior, is deferred to T032/T034 after implementation.
...
3. Manifest references include media type, byte length, SHA-256, schema version, capture ID, and retention policy. Artifact references are capped at four; manifests/structured responses at 256 KiB. Native-scene artifact and chunk metadata lengths cap at 16,777,216 bytes; raster counterparts cap at 67,108,864.
4. At commit, compute the public full SHA-256 and a server-internal SHA-256 table for fixed 65,536-byte storage chunks. Before every authorized release, verify file identity/length and re-hash every touched chunk—including both chunks of an unaligned crossing range. The internal table is never a caller-visible authority.
5. `read_capture_artifact` accepts only `debugSessionId`, `protocolVersion`, `schemaVersion`, `artifactId`, zero-based offset, and `maxBytes`; it returns at most 65,536 bytes only after those checks. Unknown, foreign, expired, deleted, and unavailable IDs all return exactly `{kind: "tool_error", tool: "read_capture_artifact", code: "ARTIFACT_NOT_FOUND", message: "Artifact is not available."}` with no additional member, artifact metadata, or free-text variance; any identity, length, or touched-chunk mismatch returns `ARTIFACT_INTEGRITY_FAILED` and no bytes.
...
**Publishing or merging this planning packet is allowed and does not constitute T001 approval. T001 operator approval applies to the exact merged candidate bytes and authorizes only M0-G0 T002–T007.** T007 must turn the approved Draft-7 schema bytes, production request/result contract validators, concrete-fixture validator parity, corpus syntax/internal-reference integrity, expected classification vocabulary, and negative structural/version cases GREEN before T008+ M0 implementation. It must not execute or claim GREEN observer, artifact, stability, atomicity, or full behavioral C001–C024 cases; T032/T034 own that evidence. Until then, no product code, test-project edit, package change, implementation command, build, test run, formatter, implementation PR, product publication, or external agent CLI is in scope.
