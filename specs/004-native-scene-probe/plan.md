...
This is a planning-only delivery. T001 records operator approval and authorizes **only** M0-G0 tasks T002–T007; no M0 primitive implementation is authorized until T007 has produced GREEN M0-G0 evidence. This packet may contain specification, research, schemas, architecture, quickstart, and unchecked task planning; it authorizes no product source/test/package change, build, formatter, workflow execution, task execution, publication, or route cutover.
...
- The capability declaration retains its `primitives` array but has exactly six entries: its Draft-7 `minItems: 6`, `maxItems: 6`, and six `allOf`/`contains` constraints require one entry for each approved name; runtime also asserts the exact six-name set with no duplicate or omission. The names are `get_ui_probe_capabilities`, `capture_visual_evidence`, `read_capture_artifact`, `wait_for_ui_stable`, `capture_element_snapshot`, and `capture_native_scene`.
- Capability output declares an explicit state for each requested settle condition—`dispatcherIdle`, `stableLayout`, `animationState`, `windowGeometry`, `contextMaterialization`, and `asyncLoadSettled`—rather than inferring support from a request. It also negotiates `sampleCount` with inclusive minimum 2 and maximum 16.
...
- Unknown, foreign, expired, deleted, and unavailable artifact IDs return exactly `{kind: "tool_error", tool: "read_capture_artifact", code: "ARTIFACT_NOT_FOUND", message: "Artifact is not available."}`—no additional member, artifact metadata, or free-text variation.
- Runtime/schema parity includes closed roots, formats, scalar/collection bounds, version syntax, the exact six-entry capability declaration, and all result/error envelopes. Runtime-only checks include process/session binding, crypto, file identity, decoded byte count, cross-field arithmetic, graph uniqueness/acyclicity, JSON depth, pre-materialization UTF-8 size, and dispatcher transaction behavior.
...
These are candidate acceptance conditions, not a claim that product code, schemas, or parity tests were changed in this documentation delivery.
...
|---|---|---|
| Explicit approval boundary | T001's durable operator Approval Record names the exact candidate schema and corpus bytes and authorizes only M0-G0 T002–T007. Primitive implementation starts only after GREEN T007 has passed. | **PASS — no implementation attempted** |
...
| Planning-only scope | This feature directory may contain the complete SpecKit planning packet, including unchecked `tasks.md` and candidate schemas/corpus. No SpecKit workflow file/run, product source/test/package edit, command execution, build, formatter, commit, push, PR, external agent CLI, or implementation task is authorized. | **PASS — planning artifacts only; all tasks unchecked** |
...
`tasks.md` is present as the requested Phase 2 planning artifact, but every item remains unchecked. T001 authorizes only M0-G0 T002–T007; T008+ remains blocked until GREEN T007. SpecKit workflow files/runs, executed commands, and generated execution reports are deliberately absent.
...
| NSP-G0-02 | Test/design gate | Prove Draft-7 schema/runtime parity and every C001–C024 corpus classification after NSP-G0-01. No primitive registration belongs here. | Every C001–C024 exchange has exactly one expected result, including C020 observer-output containment before DTO/artifact commit and C021–C024 capability omission, duplicate-name, false-revalidation, and state-size negatives; closed roots/bounds are enforced; malformed version syntax is `INVALID_TOOL_ARGUMENTS`; valid unsupported versions are `UNSUPPORTED_PROTOCOL`. |
...
| NSP-M0-02 | Code | Add the bounded native C# bridge client and server-owned artifact lifecycle after NSP-M0-01. | Focused test proves nonce/correlation, one in-flight bridge operation, cancellation cleanup, staged atomic commit, CSPRNG opaque IDs, 4-hour-or-stop retention, and typed unavailable/integrity outcomes. |
| NSP-M0-03 | Code | Add M0 lossless visual capture and bounded chunk retrieval after NSP-M0-02. | A Windows fixture produces a compact PNG manifest; read chunks reconstruct the manifest hash/length without a path/root or embedded lossless bytes. |
| NSP-M0-04 | Test | Prove M0 external behavior after NSP-M0-01 through NSP-M0-03. | RED-then-GREEN evidence covers capabilities, candidate mismatch, unavailable observer, raw PNG/preview separation, range boundaries, foreign/expired/nonexistent non-disclosure, and tamper containment. |
...
| NSP-M1-02 | Code | Add the opt-in WPF probe and immutable dispatcher transaction after NSP-M1-01. | WPF fixture proves one non-yielding materialization transaction with equal probe-owned before/after revisions before `COMPLETE` is emitted. |
| NSP-M1-03 | Code/Test | Add bounded element/scene artifacts and UIA guarded fallback after NSP-M1-01; atomic branch also depends on NSP-M1-02. | Unique/missing/ambiguous identity, geometry/DPI/clip/relations, opaque adapter facts, changed revisions, and UIA traversal during mutation result in the prescribed complete/partial/unobservable classification. |
...
|---|---|---|---|
| M0-G0 — frozen wire | The evidence contract has one reviewed byte-identical schema/corpus definition and unambiguous representative classifications; no primitive implementation has started. | NSP-G0-01, NSP-G0-02 | Recorded operator approval is mandatory; this is a contract gate, not a public release. |
...
| M1 — qualified scene facts | A bound native session can stabilize/revalidate, capture element/scene facts, distinguish an in-process WPF atomic transaction from guarded UIA, and persist bounded scene evidence. | NSP-M1-01 through NSP-M1-04 | Depends on M0; `COMPLETE` requires the WPF probe transaction; UIA remains `PARTIAL`/`UNOBSERVABLE`; raster is independently timed corroboration. |
...
2. GREEN T007 proves schema/runtime parity and every C001–C024 negative/exchange classification; only then may M0 primitive implementation T008+ start.
