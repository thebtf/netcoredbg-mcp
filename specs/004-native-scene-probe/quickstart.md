...
- request and result validators agree with the frozen schemas on concrete contract-gate fixtures, including closed roots, `sceneRequest`, visual scope/runtime-binding shape, element/native `evidenceScope: null`, scalar/collection limits, and result/error envelopes;
...
- capability declaration lists have non-empty unique supported protocol/schema version arrays containing the active `/1` values, exactly six primitives with their fixed milestones, and fixed 2/16 settle limits; the runtime validator rejects omitted active versions, duplicates, omitted primitive names, and cross-milestone pairs;
- structural negatives cover closed-root violations, invalid range/identifier, wrong tool/error-code pairs, native-scene chunk metadata above 16,777,216 bytes, raster metadata above 67,108,864 bytes, request `sampleCount` below 2 or above 16, capability `settleSampleCountMin` other than 2 or `settleSampleCountMax` other than 16, object/array with more than 256 members, nesting deeper than 16, custom payload larger than 262,144 UTF-8 bytes, and `selectedState`/`currentState` larger than 262,144 serialized UTF-8 bytes;
...
- every authorized chunk read verifies committed file identity and length, then re-hashes every touched server-internal fixed 65,536-byte chunk before releasing bytes; an unaligned range crossing chunks verifies both. A 65,536-byte request never returns more raw bytes, a native-scene chunk declares no more than 16,777,216 bytes, raster chunks no more than 67,108,864, and a terminal empty range returns `bytesRead: 0`, `dataBase64: ""`, `endOfArtifact: true`;
- foreign, unknown, expired, deleted, and unavailable IDs produce exactly `{kind: "tool_error", tool: "read_capture_artifact", code: "ARTIFACT_NOT_FOUND", message: "Artifact is not available."}` with no additional member or metadata; all other errors use only the tool-specific permitted code branch, and same-identity/same-length in-place tampering that changes any touched chunk produces `ARTIFACT_INTEGRITY_FAILED` with zero released bytes;
...
- a unique element capture has canonical identity separate from accessibility/AutomationId and includes only observed supported facts; its manifest `evidenceScope` is `null`; a `COMPLETE` element snapshot returns an `application/vnd.netcoredbg.native-scene+json`/`observed_facts` descriptor for a retrieved one-node `element_snapshot` artifact, `PARTIAL` may retain committed qualified facts, and `UNOBSERVABLE` has no artifacts; missing and ambiguous selectors return their typed outcomes;
- the opt-in WPF fixture proves one dispatcher-affine non-yielding transaction: revision-before, full bounded immutable DTO materialization, revision-after, then equal valid revisions for an atomic `COMPLETE` native-scene result whose manifest `evidenceScope` is `null` and which has at least one descriptor with `mediaType: application/vnd.netcoredbg.native-scene+json` and `evidenceGrade: observed_facts`; `PARTIAL`/`UNOBSERVABLE` may have zero artifacts only when evidence could not be committed;
- a changed revision or incomplete probe evidence becomes `PARTIAL` or `UNOBSERVABLE`, never `COMPLETE`, while retaining capture-time revalidation when it returns or commits evidence; persisted `native_scene` `PARTIAL` artifacts use only in-process or UIA-guarded atomicity, while persisted elements retain `not_applicable`;
...
5. Read the artifact in 65,536-byte or smaller chunks with `read_capture_artifact`; before each release verify identity/length and every touched 65,536-byte chunk, then concatenate decoded raw bytes and compare exact byte count and manifest SHA-256.
6. After a successful read, alter authorized artifact bytes in place without changing identity or length, including bytes in both chunks of an unaligned boundary-crossing range; repeat the read and require `ARTIFACT_INTEGRITY_FAILED` with no bytes. Also repeat with an end offset, a foreign ID, and an expired ID produced by fake clock/session stop.

Expected evidence: C005–C010 plus capability-declaration negatives C021–C022 pass; raw evidence never appears in the capture envelope; the fixed unavailable envelope discloses no artifact property; public manifest SHA-256 remains caller-visible while chunk hashes remain server-internal; integrity containment emits no bytes.
...
| Artifact reads | Requested offsets/max bytes, decoded bytes/terminal state, reconstructed hash/length, unavailable non-disclosure, and every-read identity/length plus touched-chunk verification; include a post-success same-identity/same-length tamper across an unaligned two-chunk range that returns zero bytes. |
| Lifecycle | One-in-flight/correlation and cancellation cleanup results; fake-time four-hour expiry and session-stop expiry. |
| Atomicity | Probe authority, before/after revisions, immutable DTO fixture result, the required `application/vnd.netcoredbg.native-scene+json`/`observed_facts` descriptor for every `COMPLETE` native scene and a retrievable one-node `element_snapshot` artifact for every `COMPLETE` element snapshot, UIA guarded result, and stale-wait revalidation result proving `revalidatedByCapture: true` for every evidence-returning/committing element or scene capture branch. |
