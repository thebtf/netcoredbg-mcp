...
T001 authorizes **only** M0-G0 T002–T007; it authorizes no M0 primitive implementation. T008+ remains hard-blocked until T007 has recorded GREEN M0-G0 evidence. Within the authorized M0-G0 slice, T002 verifies approved bytes and T003–T007 may perform only the contract catalog, parity, and negative-wire work. No task permits Python invocation/change/dependency, Factory or Gallery implementation, DTCG resolution/comparison, `check_element_tokens`, storage-path/root exposure, UIA atomicity claims, public cutover/release, or M2–M5 work.
...
**Goal**: prove that one approved Draft-7 contract and its corpus classify every representative exchange unambiguously before any of the six primitives is implemented or registered.

**Independent test**: replay C001–C024 from `specs/004-native-scene-probe/contracts/parity-corpus.json` against the exact approved bytes; each exchange receives exactly one expected classification, including C020 observer-output containment before DTO/artifact commit, C021 omitted primitive capability, C022 duplicate primitive name, C023 false capture revalidation after a prior wait, and C024 oversized selected/current state.
...
- [ ] T004 [P] [US1] Write failing exact-byte/schema-parity tests in `host/NetCoreDbg.Mcp.Stateless.Tests/NativeScene/NativeSceneSchemaParityTests.cs` using only the approved files under `specs/004-native-scene-probe/contracts/`; cover closed roots, `sceneRequest`, scalar/collection limits, public capability syntax, each 262,144-byte `selectedState`/`currentState` pre-materialization ceiling, and C001–C024. **Depends on**: T003. **Acceptance**: the first focused run is RED because no native runtime/catalog parity implementation exists; its receipt records the missing contract behavior without modifying schema/corpus bytes, including the exactly-six capability array (`minItems`/`maxItems` 6 and all six `allOf`/`contains` names), settle-condition states, and negotiated sample-count bounds 2–16.
- [ ] T005 [P] [US1] Write failing negative-wire and observer-output tests in `host/NetCoreDbg.Mcp.Stateless.Tests/NativeScene/NativeSceneNegativeWireTests.cs` for malformed protocol/schema syntax (`INVALID_TOOL_ARGUMENTS`), recognized unsupported versions (`UNSUPPORTED_PROTOCOL`), unavailable primitive/context (`UNSUPPORTED_CAPABILITY`), closed-object violations, invalid range/identifier, sample count outside 2–16, depth 17, more than 256 members, and a 262,145-byte custom payload. **Depends on**: T003. **Acceptance**: the first focused run is RED and demonstrates no observer, artifact, or session action before invalid input is rejected; separately C020 treats oversized observer output as `OBSERVER_UNAVAILABLE` and proves no DTO materialization or artifact commit, while C021–C024 prove capability omission, duplicate primitive name, false revalidation on a `PARTIAL` capture after a prior wait, and oversized selected/current state rejection.
- [ ] T006 [US1] Implement the exact-byte catalog and approved-version classifier in `host/NetCoreDbg.Mcp.Stateless/NativeScene/NativeSceneContractCatalog.cs`; link only the three approved artifacts as resources from `host/NetCoreDbg.Mcp.Stateless/NetCoreDbg.Mcp.Stateless.csproj`. **Depends on**: T004, T005. **Acceptance**: catalog reads the byte-identical approved resources, classifies syntax separately from unsupported versions/capabilities, and defines neither a duplicate schema nor a primitive registration.
- [ ] T007 [US1] Turn the M0-G0 suites GREEN in `host/NetCoreDbg.Mcp.Stateless.Tests/NativeScene/{NativeSceneSchemaParityTests.cs,NativeSceneNegativeWireTests.cs}` and run the focused command in `specs/004-native-scene-probe/quickstart.md`. **Depends on**: T006. **Acceptance**: C001–C024 have exactly one expected outcome; C020 contains oversized observer output before DTO/artifact commit; C021–C022 reject omitted or duplicate primitive entries despite the six-entry declaration; C023 rejects `revalidatedByCapture: false` on a `PARTIAL` capture after a prior wait; C024 rejects a selected/current-state value above 262,144 serialized UTF-8 bytes before materialization; malformed version syntax is `INVALID_TOOL_ARGUMENTS`, valid unsupported versions are `UNSUPPORTED_PROTOCOL`, runtime-only checks remain explicitly tested, and no public tool is registered. GREEN T007 is the sole authorization for T008+ M0 implementation.

**Checkpoint**: M0-G0 is accepted only with an approval record plus GREEN exact-byte parity and negative-wire evidence. GREEN T007 authorizes the next internal M0 implementation tasks, not release or public use.
...
- [ ] T008 [US1] Write failing front-door and capability tests in `host/NetCoreDbg.Mcp.Stateless.Tests/NativeScene/NativeSceneCapabilityTests.cs` using `host/NetCoreDbg.Mcp.Stateless.Tests/ModernMcp/ModernMcpProcessDriver.cs`; cover all six names exactly once, M0 availability, M1 unavailable states, protocol/schema/limit/context declaration, explicit settle-condition capability states, negotiated `sampleCount` min/max 2/16, no artifact enumeration, explicit session lookup, positive local candidate binding, remote/stale/mismatched rejection, and unchanged existing tool contracts. **Depends on**: T007. **Acceptance**: the first run is RED because no native-scene MCP dispatch exists, while retained native tools remain observable through the existing driver.
- [ ] T009 [US1] Implement native-scene request/result envelopes and the capability declaration in `host/NetCoreDbg.Mcp.Stateless/NativeScene/NativeSceneToolDispatcher.cs`, sourcing version and limit facts only from `host/NetCoreDbg.Mcp.Stateless/NativeScene/NativeSceneContractCatalog.cs`. **Depends on**: T008. **Acceptance**: only the six approved names and typed observation envelopes exist; the `primitives` array has exactly the approved six names and runtime rejects any omission or duplicate; output declares settle-condition states and negotiated 2–16 sample-count bounds, and has no `artifactId` enumeration, DTCG field, comparison/verdict, diagnosis, repair advice, or alias.
...
**Independent test**: a controlled local session creates a compact visual manifest, reconstructs the original bytes from beginning/middle/end chunks, and proves indistinguishable unavailable outcomes plus zero-byte integrity containment.

### RED tests and controlled evidence fixture

- [ ] T013 [P] [US2] Write failing artifact lifecycle, authorization, range, integrity, fake-time expiry, and session-stop cleanup tests in `host/NetCoreDbg.Mcp.Stateless.Tests/NativeScene/NativeSceneArtifactStoreTests.cs`. **Depends on**: T012. **Acceptance**: the initial test run is RED and specifies staged-versus-committed visibility, 128-bit CSPRNG base64url IDs, first-read SHA-256 verification, later identity/length verification, exactly `{kind: "tool_error", tool: "read_capture_artifact", code: "ARTIFACT_NOT_FOUND", message: "Artifact is not available."}` for unknown, foreign, expired, deleted, and unavailable IDs with no additional member, metadata, or free-text variance, `ARTIFACT_INTEGRITY_FAILED` zero-byte containment, 65,536-byte ceiling, terminal empty ranges, four-hour expiry, and stop cleanup.
...
- [ ] T016 [US2] Implement server-owned staged/atomic artifact commits, immutable descriptors, opaque capability minting, session/capture authorization, bounded padded-base64 reads, integrity checks, `TimeProvider` retention, and idempotent cleanup in `host/NetCoreDbg.Mcp.Stateless/NativeScene/NativeSceneArtifactStore.cs`. **Depends on**: T013. **Acceptance**: the store exposes no caller-selected path/root; unknown, foreign, expired, deleted, and unavailable capabilities return exactly `{kind: "tool_error", tool: "read_capture_artifact", code: "ARTIFACT_NOT_FOUND", message: "Artifact is not available."}` with no additional member, metadata, or free-text variation; authorized mismatch returns no bytes; and all file/crypto/time behavior uses BCL APIs.
...
- [ ] T028 [US3] Implement bounded element/scene capture qualification and artifact hand-off in `host/NetCoreDbg.Mcp.Stateless/NativeScene/{NativeSceneCaptureCoordinator.cs,NativeSceneToolDispatcher.cs}`. **Depends on**: T022, T026, T027. **Acceptance**: `capture_element_snapshot` uses `not_applicable` atomicity; `capture_native_scene` emits `COMPLETE` only for equal valid in-process revisions, otherwise `PARTIAL`/`UNOBSERVABLE`; every element/scene branch that returns or commits evidence has `revalidatedByCapture: true`; graph ID/parent/cycle and selected/current-state, byte/depth/custom-payload runtime bounds are enforced before commit.
- [ ] T029 [US3] Turn `host/NetCoreDbg.Mcp.Stateless.Tests/NativeScene/NativeSceneAtomicityTests.cs` GREEN and run the M1 focused command in `specs/004-native-scene-probe/quickstart.md`. **Depends on**: T028. **Acceptance**: controlled equal revisions yield one authorized `COMPLETE` atomic scene; changed revisions never do; matching UIA guards stay qualified; changed/unusable guards become unobservable; raster remains independently timed corroboration; every evidence-returning/committing element or scene result is capture-revalidated.
...
1. **Approval / M0-G0**: T001 is mandatory and authorizes only T002–T007. Those tasks establish the sole approved wire source; GREEN T007 is required before T008+.
...
1. **Approved contract increment (US1 / M0-G0)**: complete T001–T007. Stop if the byte-identical contract or any negative classification is not proven. This increment exposes no primitive.
