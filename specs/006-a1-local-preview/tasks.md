...
- [ ] T001 [P] [US2] Create RED strict-preview policy tests in `host/NetCoreDbg.Mcp.CodeSearch.Core.Tests/{NetCoreDbg.Mcp.CodeSearch.Core.Tests.csproj,PreviewSearchPolicyTests.cs}` covering core-owned verified root `.gitignore`/directory/`.cs` opens, reparse/final-path refusal, unreadable fault injection, no partial output, count/byte/result/response/deadline limits, cancellation, argument-before-I/O, and redaction. **Acceptance:** every core-owned path, I/O, resource, cancellation, and result-redaction case has a deterministic failing test with nonzero denominator; process launch/configuration/protocol/EOF cases remain T004/T007.
