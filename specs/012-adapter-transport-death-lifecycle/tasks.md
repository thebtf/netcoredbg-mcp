...
- [x] **T012 [TD-008, TD-010]** Inspect the scoped candidate diff, verify that only the planned client/manager/state/test/documentation surfaces changed, and create one atomic candidate commit. **Depends on:** T011. **Acceptance:** the commit contains the finished behavior, focused tests, and explanatory documentation; it contains no build-cleanup, Sonar, coverage, workflow, release, package, public-route, or stateless-preview change. **Evidence:** the scoped diff contains only `dap/client.py`, `session/{manager.py,state.py}`, the four focused test files, and this task ledger; focused lifecycle proof is `123 passed`, adjacent lifecycle proof is `116 passed`, and Ruff/compile/diff checks are clean. The atomic commit created by this task is the exact T013 review candidate.
