...
|---|---|---|
| B1-REQ-001/002/004/005 | MCP dispatch, exact opaque-token policy, catalog, normalized `tools/call` result wire, final structured-content serialization boundary, and frozen application-schema parity | `host/NetCoreDbg.Mcp.Stateless/Program.cs`; `ModernMcp/ModernProtocolContractTests.cs`; `ModernMcp/StructuredContentSchemaParityTests.cs`; `specs/001-mcp-stateless-strangler/contracts/modern-front-door.schema.json`; new `ModernMcp/*Threads*Tests.cs` (the process-wire and 262,144/262,145 boundary-test owner) |
