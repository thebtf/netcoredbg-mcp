...
| **COV-008** | Python coverage uses the isolated locked `uv` route and `.coveragerc` source policy without permanent dependency changes. | The Python final report has positive line and branch denominators and only valid `src/netcoredbg_mcp` mappings. |
...
| **COV-010** | Each fixed .NET producer restores and tests without `--no-build`, filters, exclusions, thresholds, or merge switches, and writes its planned private Cobertura input. The Preview test producer receives a deterministic local source-run archive/manifest below the claimed root because its consumer test intentionally refuses repository build output. | A zero-exit missing input blocks with `COVERAGE_REPORT_MISSING`; the local Preview fixture is hash-checked, explicitly located, and removed with the coverage root. |
