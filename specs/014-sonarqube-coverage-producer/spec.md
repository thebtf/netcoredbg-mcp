...
| **COV-013** | The runner validates the Python final report for Cobertura shape, positive denominators, containment, canonical source-root resolution, and source-set uniqueness. | Missing, malformed, zero, absolute class filenames, URI/escaping/reparse-backed source roots, duplicate source-set identities, test-only mappings, or unsafe paths block. |
