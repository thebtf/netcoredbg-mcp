...
**Release intent**: `none`.
**Execution gate**: Wave-2 PR #289 is open. T000 must discover the tracked `specs/013-owner-scoped-prebuild-cleanup/wave-closure-v1.json`, validate source `integration.kind: pull_request_head`, `release_intent: none`, accepted candidate, receipt hash, and PR head identity, then separately prove PR #289 merged, derive runtime `observed_main_sha`, and prove candidate/artifact ancestry before T001 through T028 may implement, run a producer, begin a scanner, claim a root, or invoke diagnostic.
...
| --- | --- | --- | --- |
| **R01 / V01** | In a clean-clone or hosted-workspace fixture, omit or untrack the Wave-2 artifact; alter source kind, receipt hash, release intent, candidate SHA, PR head identity, merge proof, artifact ancestry, or runtime observed-main ancestry. Current runner has no Wave-2 gate. | `WAVE2_CLOSURE_UNVERIFIED` occurs before preflight, begin, and claim; only after claim does the runner write a hash-bound resolved copy with `observed_main_sha` inside its own run root. | T000, T001, T012, T013, T023 |
...
| COV-002 | T000 to T001, T012 to T013, T028 | Typed Wave-2 candidate/PR source plus runtime observed-main entry. |
...
- [ ] **T000 [ENTRY]** Treat `specs/013-owner-scoped-prebuild-cleanup/wave-closure-v1.json` as an external Wave-2 PR-head prerequisite. After Wave-2 T014 creates it on PR #289 and that PR merges, validate source `integration.kind: pull_request_head`, `release_intent: none`, closure receipt hash, accepted candidate, and PR head-ref/SHA. Separately prove PR merge through GitHub/workflow evidence or first-parent history, derive `artifact_commit_sha` and `observed_main_sha`, then validate candidate/artifact ancestry to observed main. **Requirements**: COV-002. **Acceptance**: only the merged tracked source plus runtime merge proof admits Wave 3; while PR #289 remains open, return `WAVE2_CLOSURE_UNVERIFIED` without starting Wave-3 implementation or diagnostic action.

## Slice S1: behavior-first RED matrix

- [ ] **T001 [S1]** Add R01 and R02 entry/preflight event-spy tests, including a clean-clone/hosted-workspace fixture with no `.agent` entry state. **Requirements**: COV-002, COV-004, COV-023, COV-026. **Acceptance**: all invalid entry and toolchain inputs leave begin and claim at zero.
...
- [ ] **T012 [S3]** Implement tracked Wave-2 PR-head entry resolution, source schema/release-intent/PR-head validation, runtime merge/observed-main/ancestry proof, and `preflight_coverage_toolchain` before scanner begin. **Requirements**: COV-001, COV-002, COV-004, COV-009, COV-026. **Acceptance**: R01 and R02 turn green with zero begin/claim on failure while existing role CLI shapes remain unchanged.
- [ ] **T013 [S3]** Implement `CoveragePlan`, two-final-report marker derivation, tracked PR-head source binding, runtime `artifact_commit_sha` and `observed_main_sha`, a run-root resolved entry copy after claim, five private input paths, and exclusive claim. **Requirements**: COV-002, COV-003, COV-005. **Acceptance**: plan is pure; no run-local entry exists before claim; marker schema v1 validates.
...
- [ ] **T027 [S4]** Bind review and judgment to the exact diagnostic scanner head. **Requirements**: COV-002, COV-022, COV-024 to COV-026. **Acceptance**: tracked source, runtime observed-main proof, frozen SHA, reviewer, judge, and scanner worktree agree.
