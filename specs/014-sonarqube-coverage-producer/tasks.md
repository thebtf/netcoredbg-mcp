...
- [ ] **T010 [S2]** Add per-project restore/test Cobertura-input commands without prohibited switches. **Requirements**: COV-010. **Acceptance**: each producer writes its planned private input or fails before end. **Current evidence**: command contracts and initial real projects run; the Preview project exposed and received a deterministic local source-run artifact prerequisite plus identity-collision correction. One final full producer rerun remains the exact acceptance gate.
