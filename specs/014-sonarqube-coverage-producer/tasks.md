...
**Release intent**: `none`.
**Execution gate**: Wave-2 PR #289 is open. T000 must discover the tracked `specs/013-owner-scoped-prebuild-cleanup/wave-closure-v1.json`, validate schema, `release_intent: none`, accepted candidate, receipt hash, and PR head identity, then derive runtime `observed_main_sha` and prove PR merge plus candidate/artifact ancestry before T001 through T028 may implement, run a producer, begin a scanner, claim a root, or invoke diagnostic.
...
| --- | --- | --- | --- |
| **R01 / V01** | In a clean-clone or hosted-workspace fixture, omit or untrack the Wave-2 artifact; alter its receipt hash, release intent, candidate SHA, PR head identity, merge proof, artifact ancestry, or runtime observed-main ancestry. Current runner has no Wave-2 gate. | `WAVE2_CLOSURE_UNVERIFIED` occurs before preflight, begin, and claim; only after claim does the runner write a hash-bound resolved copy with `observed_main_sha` inside its own run root. | T000, T001, T012, T013, T023 |
...
- [ ] **T000 [ENTRY]** Treat `specs/013-owner-scoped-prebuild-cleanup/wave-closure-v1.json` as an external Wave-2 prerequisite. After Wave-2 T014/PR #289 merges it, validate its schema, `release_intent: none`, closure receipt hash, accepted candidate, and PR head-ref/SHA. Derive `artifact_commit_sha` and `observed_main_sha`, prove PR merge through GitHub/workflow evidence or first-parent history, then validate candidate/artifact ancestry to observed main. **Requirements**: COV-002. **Acceptance**: only the merged tracked artifact plus runtime proof admits Wave 3; while PR #289 remains open, return `WAVE2_CLOSURE_UNVERIFIED` without starting Wave-3 implementation or diagnostic action.
