...
public static partial class GridCommands
{
    ...
    public static JsonNode DoubleClickRow(JsonNode? @params, UIA3Automation automation, AutomationElement? mainWindow)
    {
        if (JsonRpcHandler.Stealth)
        {
            return Blocked(
                "grid row double click requires foreground mouse input",
                new JsonObject { ["mode"] = "stealth" },
                new JsonObject { ["mode"] = "non-stealth FlaUI bridge" },
                "Disable stealth mode or use a non-coordinate UIA action.");
        }
