...
| `#268` | Target slice merged; broader FR remains open | PR #90; `runtime_smoke_tail_events` registered and lifecycle coverage merged; CR-012 adds first-class `runtime_smoke_validate_plan` without launching target apps; CR-013 adds validated `runtime_smoke_run_plan` and bounded `runtime_smoke_evidence_bundle` target evidence | Keep or split `runtime_smoke.run_probe`, oracle-pack integration, and app diagnostics executable work before closing the full FR. |
| `#269` | Target slice merged; broader FR remains open | PR #90; `runtime_smoke_get_result` and `runtime_smoke_stop` registered and lifecycle coverage merged; CR-013 adds the evidence-bundle API for compact result/event/evidence metadata | Keep or split `agent_mode`, event cursor/delta ergonomics, `run_probe`, and evidence-first probe UX before closing the full FR. |
