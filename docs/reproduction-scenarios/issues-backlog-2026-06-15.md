...
| `#271` | Schema slice merged; executable diagnostic slice covered; broader FR remains open | PR #91; diagnostic schema `netcoredbg.runtime_smoke.diagnostics.v1`, oracle/app examples, and focused schema/docs/contracts tests merged; CR-015 adds executable `oracle_pack` and `app_diagnostics` probe evidence for the bounded diagnostics slice | Keep or split executable `debug_preflight`, tracepoint guard, trace cursor/delta, cleanup contract, contamination, and single-flight enforcement work before closing the full FR. |
| `#272` | Schema slice merged; executable diagnostic slice covered; broader FR remains open | PR #91; semantic-probe registry contract and tracepoint guardrail vocabulary merged; CR-015 adds bounded app diagnostics and `oracle_pack` run-probe/evidence-bundle execution | Keep or split app diagnostics polling/waiting and broader `oracle_pack` orchestration before closing the full FR. |
...
| `#271/#272` | `tests/test_runtime_smoke_diagnostics_schema.py`, `tests/test_runtime_smoke_v2_docs.py`, `tests/test_runtime_smoke_v2_probes/test_oracle_pack.py`, and `tests/test_runtime_smoke_v2_probes/test_app_diagnostics.py` | CR-007 RED: `uv run pytest --runxfail tests/test_runtime_smoke_diagnostics_schema.py tests/test_runtime_smoke_v2_docs.py -q`; CR-015 RED: `uv run pytest --runxfail tests/test_runtime_smoke_v2_probes/test_oracle_pack.py tests/test_runtime_smoke_v2_probes/test_app_diagnostics.py tests/test_runtime_smoke_run_probe_facade.py -q` | Strengthened schema/docs contracts failed before CR-007; executable `oracle_pack` / `app_diagnostics` probe behavior failed before CR-015. |
