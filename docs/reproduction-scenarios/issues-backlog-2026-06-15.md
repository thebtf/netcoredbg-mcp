...
| `#268` | Target slice merged; broader FR remains open | PR #90; `runtime_smoke_tail_events` registered and lifecycle coverage merged; CR-012 adds first-class `runtime_smoke_validate_plan` without launching target apps | Keep or split first-class `runtime_smoke.run_plan` and evidence-bundle API work before closing the full FR. |
...
| `#268` | `tests/test_runtime_smoke_registration.py::test_runtime_smoke_agent_lifecycle_tools_are_registered`; `tests/test_runtime_smoke_registration.py::test_runtime_smoke_validate_plan_tool_is_registered` and `tests/test_runtime_smoke_plan_facade.py` | Lifecycle RED same as `#269`; CR-012 RED: `uv run pytest --runxfail tests/test_runtime_smoke_registration.py::test_runtime_smoke_validate_plan_tool_is_registered tests/test_runtime_smoke_plan_facade.py -q` | `runtime_smoke_tail_events` was not registered; later `runtime_smoke_validate_plan` was not registered. |
