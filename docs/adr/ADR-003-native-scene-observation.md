...
| `capture_element_snapshot` | M1 | Capture one uniquely resolved element's observed facts and relation metadata; a `COMPLETE` response contains a retrievable one-node scene artifact, while `PARTIAL` may retain qualified facts and `UNOBSERVABLE` has none. |
...
Artifacts become immutable once committed. At commit, the server computes the full manifest SHA-256 and a server-internal SHA-256 table for fixed 65,536-byte storage chunks. Every authorized read verifies immutable file identity and length, then re-hashes every chunk touched by the requested range—including both chunks for an unaligned crossing range—before emitting any bytes. A mismatch yields `ARTIFACT_INTEGRITY_FAILED` and no bytes. The full manifest SHA remains public; the chunk table is not exposed or caller authority. A valid session requesting an unknown, foreign-session, foreign-capture, expired, deleted, or otherwise unavailable capability receives the indistinguishable `ARTIFACT_NOT_FOUND` result with no existence, ownership, retention, size, hash, or path disclosure. Integrity failure is separate because it concerns an already authorized owned artifact and must trigger containment rather than masquerade as missing data.
