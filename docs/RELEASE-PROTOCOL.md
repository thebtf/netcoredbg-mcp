...
- Paginate `OPEN`, `CONFIRMED`, and `REOPENED` findings completely; every baseline or discovered key must be absent only after source code is fixed and the affected scan is repeated.
