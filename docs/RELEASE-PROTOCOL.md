...
exact-head gate is `python scripts/run_sonarqube_exact_head.py`; it receives
credentials only from its runtime environment, rejects any in-tree `.env` and
`SONAR_ADMIN_TOKEN`, keeps Sonar credentials out of build/test children, and
writes the role-aware secret-free receipt under the Git-common coordination
root.
