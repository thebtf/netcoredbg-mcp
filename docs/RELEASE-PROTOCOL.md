...
the parent of `git rev-parse --git-common-dir`; explicit allowed process values
override that file. It rejects `SONAR_ADMIN_TOKEN`, any other unsupported
`SONAR_` credential name, and a scanner worktree containing `.env`; it scrubs
Sonar credentials from build/test children, redacts scanner argv/output, and
...
5. Write one secret-free receipt per scan containing the scan role, captured SHA, post-scan SHA, scanner-worktree cleanliness evidence, fixed project-key comparisons, task-report task/server-origin comparisons, scanner revision, submitted task, bounded Compute Engine polling outcome, analysis ID, analysis-bound quality gate, serialized project-analysis ID/revision readbacks before and after the exact-head finding pagination, baseline/discovered finding-key inventories, complete page evidence, and final empty-result/pagination-complete result.
