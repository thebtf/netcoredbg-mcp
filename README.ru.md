...
**135 MCP-инструментов · 8 промптов · 4 ресурса · 2101 собранных тестов · релиз v0.23.3**
...
## Что нового в v0.23.3

- **Отказ при почти чёрном снимке** — снимки с почти полностью чёрным кадром
  теперь возвращают `PROBABLE_BLACK_FRAME` с метриками яркости, сведениями о
  попытке изменения переднего плана и явным восстановлением через
  `ui_bring_to_front`. Аннотированный снимок перед таким ответом также
  сбрасывает кэш аннотаций.
- **Надёжное восстановление переднего плана** — при необходимости
  `ui_bring_to_front` использует резервный механизм pywinauto для
  восстановления переднего плана по HWND и отключает stealth mode только после
  успешной активации.
- **Безопаснее очистка временных файлов при запуске** — GC устаревших сессий
  отбрасывает чужие имена до проверки метаданных каталога и не выполняет
  проверки типа или метаданных для записей вне собственного префикса.
