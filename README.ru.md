...
**135 MCP-инструментов · 8 промптов · 4 ресурса · 2032 собранных тестов · релиз v0.23.1**
...
- **Preview совместимого .NET host для исходников** — новый stdio-процесс на
  .NET 8 в `host/NetCoreDbg.Mcp.Host` создаёт собственную upstream
  MCP-клиентскую сессию к Python, сам локально отвечает на downstream
  рукопожатие `initialize`, а затем проксирует реальные обмены `tools/list`
  и `tools/call` в ту же upstream-сессию. Он собирается только из рабочей
  копии исходников (не входит ни в PyPI wheel, ни в опубликованную точку
  входа) и пока не проксирует client roots, progress/log-уведомления,
  resources, prompts и другие callbacks.
- **Усиленный запуск из рабочей копии исходников** — раздел «Запуск из
  локальной рабочей копии» ниже теперь описывает однократный подготовительный
  шаг `uv sync --locked`, после которого повторные supervised-запуски
  `uv run --no-sync --project ...` перезапускают сервер, не изменяя общее
  окружение `.venv`, так что вызывающая .NET-рабочая папка остаётся корнем
  debug-проекта при каждом перезапуске.
