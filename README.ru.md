...
копии разработчика сначала один раз синхронизируйте окружение, а затем
регистрируйте долгоживущий сервер:

```powershell
cd D:\Dev\netcoredbg-mcp
uv sync --locked
# Если также нужны инструменты для тестирования: `uv sync --locked --extra dev`.
