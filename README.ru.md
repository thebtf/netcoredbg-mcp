...
**135 MCP-инструментов · 8 промптов · 4 ресурса · 2105 собранных тестов · релиз v0.23.4**
...
## Что нового в v0.23.4

- **Надёжная работа с WPF-подменю** — `ui_key_sequence` и `ui_invoke` теперь
  описывают нужную последовательность: отправьте scoped `ENTER` родительскому
  `MenuItem`, заново найдите popup-элемент и отдельно вызовите этот точный
  дочерний элемент. Один вызов родительского пункта не гарантирует появления
  соседних элементов подменю.
