...
**135 MCP-инструментов · 8 промптов · 4 ресурса · 2129 собранных тестов · релиз v0.23.6**
...
## Что нового в v0.23.6

- **Lossless screenshot evidence по запросу** — `ui_take_screenshot` сохраняет
  обычный navigation preview и добавляет session-scoped режим evidence с
  исходным PNG, SHA-256, метаданными capture/DPI/геометрии и опциональным
  crop, полученным из raw-данных. Inline preview предназначен только для
  навигации и не является acceptance evidence. DOM/WPF-метрики, pixel diff,
  калибровка модели и operator sign-off в релиз не входят.
