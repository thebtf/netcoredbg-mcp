...
**135 MCP-инструментов · 8 промптов · 4 ресурса · 2092 собранных теста · релиз v0.23.2**
...
## Что нового в v0.23.2

- **Ограниченное ожидание MCP roots** — опубликованный Python-сервер теперь
  ждёт ответ клиента на `roots/list` не более двух секунд. Область, явно
  закреплённая через переменную окружения или `--project`, сохраняет приоритет;
  допустимые локальные client roots идут перед startup CWD; сетевые и UNC-корни
  отклоняются; молчащий клиент больше не блокирует инструмент и очистку, а
  приводит к переходу на настроенный startup CWD (Engram № 380).
- **Безопасный fallback управляемого FlaUI bridge** — если другой активный
  сеанс мешает пересобрать управляемый bridge, resolver сохраняет приоритет
  последней успешно собранной версии над неконтролируемыми кандидатами из PATH
  и не допускает скрытого отката протокола.
- **Живые подписки на ресурсы** — установленная Python-точка входа теперь
  объявляет `resources.subscribe=true`, поддерживает идемпотентные subscribe и
  unsubscribe для всех четырёх ресурсов `debug://` и отправляет упорядоченные,
  объединённые `notifications/resources/updated` при изменении состояния,
  точек останова, вывода или потоков.
- **Завершённый preview .NET front door для исходников** — совместимый host на
  .NET 8 теперь покрывает tools, native prompts, downstream roots, resources и
  subscriptions, progress/logging notifications и разрешённые метаданные
  `x-mux`, а Python остаётся исполнителем. Host по-прежнему собирается только
  из исходников: он не входит в PyPI wheel, не опубликован как точка входа и не
  выполняет cutover.
