...
**135 MCP-инструмента · 8 промптов · 4 ресурса · 2032 собранных тестов · релиз v0.23.1**
...
- **Preview совместимого .NET host для исходников** — новый stdio-процесс на
  .NET 8 в `host/NetCoreDbg.Mcp.Host` проксирует реальные MCP-обмены
  `initialize`, `tools/list` и `tools/call` к этому же неизменному, всё ещё
  основному Python-серверу. Он собирается только из рабочей копии исходников
  (не входит ни в PyPI wheel, ни в опубликованную точку входа) и пока не
  проксирует client roots, progress/log-уведомления, resources, prompts и
  другие callbacks.
